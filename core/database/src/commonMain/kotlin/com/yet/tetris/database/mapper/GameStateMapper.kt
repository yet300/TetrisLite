...
fun CurrentGameState.toDomain(boardCells: List<BoardCells>): GameState {
    // Reconstruct board
    val cells = boardCells.associate { cell ->
    ...
    val currentPiece = currentPieceType?.let { type ->
...
fun GameState.toEntities(): GameStateEntities {
    val boardCells = board.cells.map { (position, type) ->
        BoardCells(
    ...
    val gameStateData = CurrentGameStateData(
        score = score,
        linesCleared = linesCleared,
