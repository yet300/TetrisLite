...
fun GameBoard.toDto(): GameBoardDto = GameBoardDto(
    width = width,
    height = height,
    cells = cells.mapKeys { it.key.toDto() }.mapValues { it.value.toDto() }
)

fun Tetromino.toDto(): TetrominoDto = TetrominoDto(
    type = type.toDto(),
...
fun TetrominoType.toDto(): TetrominoTypeDto = when (this) {
...
fun Difficulty.toDto(): DifficultyDto = when (this) {
...
fun GameBoardDto.toDomain(): GameBoard = GameBoard(
    width = width,
    height = height,
    cells = cells.mapKeys { it.key.toDomain() }.mapValues { it.value.toDomain() }
)

fun TetrominoDto.toDomain(): Tetromino = Tetromino(
    type = type.toDomain(),
...
fun TetrominoTypeDto.toDomain(): TetrominoType = when (this) {
